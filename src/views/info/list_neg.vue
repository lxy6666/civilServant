<template>
  ...
  <div class="app-container index neg">
    ...
    >
      ...
      >
        ...
        <el-form-item label="负面指标" prop="zbname" class="zbItem">
            <treeselect
              :value="addForm.zbid"
              :multiple="false"
              :options="nodes"
        ...
        <el-form-item label="处罚措施"  class="zbItem2">
          <el-input
            type="textarea"
            v-model="addForm.cfcs"
            placeholder="请输入处罚措施"
            :rows="2"
          ></el-input>
        </el-form-item>
        <el-form-item label="处罚依据"  class="zbItem2">
          <el-input
            type="textarea"
            v-model="addForm.cfyj"
            placeholder="请输入奖励依据"
            :rows="2"
          ></el-input>
        </el-form-item>
        <el-form-item label="决定书编号"  class="addItem">
          <el-input
            v-model="addForm.jdsbh"
            placeholder="请输入决定书编号"
          ></el-input>
        </el-form-item>
        <el-form-item label="违法证据"  class="addItem">
          <el-input
            v-model="addForm.wfyj"
            placeholder="违法证据"
          ></el-input>
        </el-form-item>
        <el-form-item label="执行情况"  class="addItem">
          <el-input
            v-model="addForm.zxqk"
            placeholder="执行情况"
          ></el-input>
        </el-form-item>
  ...
  data() {
    return {
      ...
      addRules: {
        ...
        jdsbh: [
          { required: true, message: "请输入决定书编号", trigger: "blur" },
        ],
        wfyj: [
          { required: true, message: "请输入违法证据", trigger: "blur" },
        ],
        zxqk: [
          { required: true, message: "请输入执行情况", trigger: "blur" },
        ]
      },
    };
  },
  ...
  methods: {
    uploadss(){
      ...
      pishen(a,val){
          ...
          postAction(`${_URL.fm_piliang}`,obj).then((response) => {
    ...
    getStatusText(val){
     ...
     getExcel(){
      let that = this;
      let url = _URL.excelURL+"toExcel/fm"
      let querys = ''
    ...
    getExcelMb(){
        window.location.href="http://222.132.158.254:7000/govent/template/负面信息模板.xls"
    },
    getList() {
      // that.listLoading = true
      let that = this;
      ...
      getAction(`${_URL.fm_findAll}`,query).then((response) => {
        console.log(JSON.stringify(response));
        if (response.flag) {
    ...
    add(formName,status) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(that.uploadType=='add'){
              postAction(`${_URL.fm_save}`, that.addForm).then((response) => {
                console.log(JSON.stringify(response))
                if (response.flag) {
                  that.$message.success(response.message);
                  //清空数据
              ...
              putAction(`${_URL.fm_update}`, that.addForm).then((response) => {
                console.log(JSON.stringify(response))
                if (response.flag) {
                  that.$message.success(response.message);
                  //清空数据
    ...
    cdelete(id) {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        type: "warning",
          ...
          deleteAction(`${_URL.fm_delete}`, obj).then((response) => {
            if (response.flag) {
              that.$message.success(response.message);
              //清空数据
              that.getList();
