<template>
  <div class="app-container index vol">
    ...
    >
      ...
      <el-table-column prop="rxzsj" label="任现职时间"> </el-table-column>
      <el-table-column prop="dept" label="部门及职务"> </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="300">
        <template slot-scope="scope" >
          <el-button type="primary" size="mini" @click="goChange(scope.row,false,false)"
            >查看</el-button
          >
          <el-button  type="primary" size="mini" @click="goChange(scope.row,true,false)"
            >编辑</el-button
          >
    ...
    >
      ...
      >
        ...
        <el-form-item label="任现职时间"  prop="zytime" class="addItem">
          <el-date-picker
            v-model="addForm.rxzsj"
            type="date"
            style="width:300px"
            placeholder="任现职时间"
            @change = "setDqtime"
        ...
        <el-form-item label="部门及职务"  prop="zynr" class="zbItem">
          <el-input
         ...
         <el-form-item label="考核等次" prop="zjhm" class="addItem">
          <el-input
            v-model="addForm.khdc"
            placeholder="考核等次"
          ></el-input>     
        </el-form-item>
         <el-form-item label="考核年度" prop="zjhm" class="addItem">
          <el-input
            v-model="addForm.khyear"
            placeholder="考核年度"
          ></el-input>     
        </el-form-item>
         <el-form-item label="考核意见" prop="zjhm" class="addItem">
          <el-input
            v-model="addForm.khyj"
            placeholder="考核意见"
          ></el-input>     
        </el-form-item>   
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      addRules: {
        ...
        rxzsj: [
          { required: true, message: "请输入任现职时间", trigger: "blur" },
        ],
        dept: [
          { required: true, message: "请输入部门职务", trigger: "blur" },
        ],
        khdc: [
        ...
        khyj: [
          { required: true, message: "请输入考核意见", trigger: "blur" },
        ],
      },
    };
  },
  ...
  methods: {
    ...
    getList() {
      ...
      //所属地点,若有则加入查询条件
      getAction(`${_URL.personKaohe_findAll}`,query).then((response) => {
        console.log(JSON.stringify(response));
        if (response.flag) {
    ...
    add(formName,status) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(that.uploadType=='add'){
              postAction(`${_URL.personKaohe_save}`, that.addForm).then((response) => {
                console.log(JSON.stringify(response))
                if (response.flag) {
              ...
              putAction(`${_URL.personKaohe_update}`, that.addForm).then((response) => {
                console.log(JSON.stringify(response))
                if (response.flag) {
                  that.$message.success(response.message);
                  //清空数据
    ...
    cdelete(id) {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        type: "warning",
          ...
          deleteAction(`${_URL.personKaohe_delete}`, obj).then((response) => {
            if (response.flag) {
              that.$message.success(response.message);
              //清空数据
              that.getList();
