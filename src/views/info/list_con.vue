<template>
  <div class="app-container index con">
    ...
    >
      ...
      <el-table-column prop="jkmoney" label="捐款金额"> </el-table-column>
      <el-table-column prop="jktime" label="捐款时间"> </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="捐款金额" prop="jkmoney" class="addItem">
          <el-input
            v-model="addForm.jkmoney"
            placeholder="请输入捐款金额"
          ></el-input>
        </el-form-item>
        <el-form-item label="捐款日期"  prop="jktime" class="addItem">
          <el-date-picker
            v-model="addForm.jktime"
            type="date"
            style="width:300px"
            placeholder="选择捐款日期"
        ...
        <!-- <el-form-item label="审核部门" prop="shbmname" class="addItem">
          <el-select
            v-model="addForm.shbmname" 
            placeholder="请选择审核部门"
            style="width: 300px"
          >
            <el-option
  ...
  data() {
    return {
      ...
      addRules: {
        ...
        jktime: [
          { required: true, message: "请输入捐款日期", trigger: "blur" },
        ],
        shbmname: [
  ...
  methods: {
    uploadss(){
     ...
     pishen(a,val){
        ...
        postAction(`${_URL.jk_piliang}`,obj).then((response) => {
    ...
    goSh(){
      ...
      setDqtime(val){
        ...
        let that = this;
        //日期对象转2021/66/88
        let jktime = val.toLocaleDateString();
        //处理
        that.addForm.jktime = jktime
      },
      // 选择了人员
      getVals(Array){
        let that = this;
    ...
    getStatusText(val){
     ...
     getExcel(){
      let that = this;
      let url = _URL.excelURL+"toExcel/jk"
      let querys = ''
    ...
    getExcelMb(){
        window.location.href="http://222.132.158.254:7000/govent/template/公益捐款模板.xlsx"
    },
    getList() {
      // that.listLoading = true
      let that = this;
      ...
      getAction(`${_URL.jk_findAll}`,query).then((response) => {
        console.log(JSON.stringify(response));
        if (response.flag) {
    ...
    add(formName,status) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(that.uploadType=='add'){
              postAction(`${_URL.jk_save}`, that.addForm).then((response) => {
                console.log(JSON.stringify(response))
                if (response.flag) {
                  that.$message.success(response.message);
                  //清空数据
              ...
              putAction(`${_URL.jk_update}`, that.addForm).then((response) => {
                console.log(JSON.stringify(response))
                if (response.flag) {
                  that.$message.success(response.message);
                  //清空数据
    ...
    cdelete(id) {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        type: "warning",
          ...
          deleteAction(`${_URL.jk_delete}`, obj).then((response) => {
            if (response.flag) {
              that.$message.success(response.message);
              //清空数据
              that.getList();
